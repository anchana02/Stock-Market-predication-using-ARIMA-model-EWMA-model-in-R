...
df=read.csv("D:\\VIT\\SEM 5\\Data Analytics\\J component\\dataset\\Extracted\\c_msft.csv")
df
...
dfM = xts(df$Close, order.by=as.Date(df$Date))
dfM
class(dfM)
par(mfrow=c(1,1))
plot(dfM)
...
par(mfrow=c(1,2))
Acf(dfM,main="ACE for Differenced Series")    #gives q value
#here we observe that the first value is 1 and trends downwards 

Pacf(dfM, main = "PACF for Differeced Series")   #->gives p value
...
auto.arima(train,seasonal = FALSE) #0,0,1
#pdq -> 1,0,23
...
fit2=arima(train,order = c(1,0,23))
tsdisplay(residuals(fit2),lag.max = 40,main="(1,0,23) Model Residuals")
#if it busts out the blue line earlier its a weak model, if it doesn't its a strong model
#ORIGINAL DATA auto.arima
fitA=auto.arima(dfM, seasonal = FALSE)
tsdisplay(residuals(fitA),lag.max = 40,main = 'Training Dataset (5,2,0) Model Residuals')

#ORIGINAL DATASET arima
fitB=arima(dfM,order = c(11,0,6))
tsdisplay(residuals(fitB),lag.max = 30,main="Training dataset (11,0,6) Model Residuals")
